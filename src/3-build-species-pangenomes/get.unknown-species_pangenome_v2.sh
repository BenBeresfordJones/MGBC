...
Get all gene annotations for a given representative genome unknown taxonomy

OPTIONS:

Input [REQUIRED]:
   -i      Representative genome without .fna or .fa extension
...
then
     echo "$(timestamp) WARNING : No output format specified so using specified representative genome id in the working directory."
     OUTDIR=$(echo "$REP_GENOME").$HOST_ORG
     echo "$(timestamp) INFO : Writing output to $OUTDIR"
elif [ -z $OUTDIR ]
then
     WRITEDIR=$(echo "$REP_GENOME").$HOST_ORG
...
# link to unknown rep databases for human and mouse
H_REPMEMS=/lustre/scratch118/infgen/team162/bb11/Human_mouse_comparison/FUNCTIONAL_COMPARISON/BUILD_PANGENOME/GTDB_DBs/human_rep_members.tsv
M_REPMEMS=/lustre/scratch118/infgen/team162/bb11/Mouse_genomes/MCGC/DREP_950/drep_950_index-ALL.csv
...
then
     ...
     # get genome ids
     grep -w "$REP_GENOME" $H_REPMEMS | cut -f1 > $OUTDIR/genome_ids.txt
...
then
     ...
     MCOUNT=$(grep -w "$REP_GENOME" $M_REPMEMS | cut -d, -f2 | grep -cFwf - $M_REPMEMS)
     ...
     # get genome ids
     grep -w "$REP_GENOME" $M_REPMEMS | cut -d, -f2 | grep -Fwf - $M_REPMEMS | cut -d, -f1 | sed 's/.fa//g' | sed 's/.fna//g' > $OUTDIR/genome_ids.txt
...
then
     ...
     SPECIES_NAME=$(echo $REP_GENOME.$HOST_ORG)
